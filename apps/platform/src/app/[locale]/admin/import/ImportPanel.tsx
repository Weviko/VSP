...
import type { ValidationResult } from '@vsp/core-admin';
import { FileField, type FileValue } from '@/components/FileField';
import { validateAction, commitAction } from './actions';

/**
 * 엑셀 일괄 등록 패널.
 *   업로드 → 검증(미리보기, 저장 안 함) → 확정 등록.
 * 검증에서 통과한 행만 등록되고, 오류·중복 행은 표에 이유와 함께 남는다.
 */
export function ImportPanel({
  ...
  function validate() {
    ...
    start(async () => {
      try { setResult(await validateAction(locale, file.id)); }
  ...
  function commit() {
    ...
    setErr(null);
    start(async () => {
      try { setDone(await commitAction(locale, file.id)); setResult(null); }
      catch (e) { setErr(e instanceof Error ? e.message : String(e)); }
    });
  }
            ...
            <table className="w-full text-sm">
              <thead className="border-b border-slate-200 bg-slate-50 text-left text-slate-600">
                <tr>
                  ...
                  <th className="px-3 py-2 font-medium">{L('gender')}</th>
                  <th className="px-3 py-2 font-medium">{L('birth')}</th>
              ...
              <tbody>
                {result.rows.map((r) => (
                  <tr key={r.row} className={'border-b border-slate-100 last:border-0 ' + (r.ok ? '' : 'bg-red-50/40')}>
                    ...
                    <td className="px-3 py-1.5 font-medium text-slate-900 wrap-anywhere">{r.full_name || '—'}</td>
                    <td className="px-3 py-1.5 text-slate-600">{r.gender ?? '—'}</td>
                    <td className="px-3 py-1.5 tabular-nums text-slate-600">{r.birth_date ?? '—'}</td>
